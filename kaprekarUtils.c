...
//ALL EDITS WERE MADE ON JANUARY 4TH, 2018 BY MARIO GOMEZ AND AIDAN JOHNSON
...
int isKaprekar(int n) {
  if(n < 1) {
    return 0;
    //EDIT: REPLACED FALSE WITH ZERO
  }

  int i;
  long square = n * (long) n;
  int numberOfDigits = (int) log10(square) + 1;
  //EDIT: CHANGED numDigits TO numberOfDigits
  //EDIT: CHANGED log10(n) TO log10(square)
  long modulus = 1;
  //EDIT: SET MODULUS EQUAL TO 1 INSTEAD OF 0
  long first, second;
  ...
  for(i=1; i<=numberOfDigits; i++) {
    ...
    modulus *= 10;
    //EDIT: CHANGED modulous TO modulus
